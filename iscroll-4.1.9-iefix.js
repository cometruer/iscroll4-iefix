 ...
 * 用法：
    var myScroll = new iScroll('wrapper', { 
		...
		hideScrollbar: false,
		fadeScrollbar: false,
		scrollbarClass: 'commonScrollBar'
	});
 */
...
iScroll.prototype = {
    ...
    //绑定滚动条
    _bindScrollBar: function(bar) {
    	if (!$) {
    		console.log('请先引入jquery');
    		return false;
    	}
        ...
        var oy = 0; //原始坐标
        var currentY = 0;
        that.choose = false;//是否选中状态
        that.barPosY = 0;//滚动条实际坐标

        $(bar).on('mousedown', function(event) {
        	// console.log('mousedown')
        	that.choose = true;
        	currentY = that.barPosY;
            oy = parseInt(event.pageY);
            //ie下不支持，并且会抢flash的全屏点击事件
        ...
        $(document).on('mousemove', function(event) {
            if (that.choose) {
				...
				var realY = currentY + newY;

                //fix
                realY = realY <= 0 ? 0 : realY;
                realY = realY >= that.vScrollbarMaxScroll ? that.vScrollbarMaxScroll : realY;

            	// console.log('滚动条y坐标=' + realY);
            	that._simpleBarMove(realY);

                var listMoveY = that.maxScrollY * that.barPosY / that.vScrollbarMaxScroll;
                //console.log('列表移动=' + listMoveY);
                //因为 realY = this.y * that.vScrollbarProp
                that.scrollTo(0, listMoveY);
                //ie下不支持，并且会抢flash的全屏点击事件
                // event.preventDefault();
                // event.stopPropagation();
            }
